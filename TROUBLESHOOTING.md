...
- [로직](#로직)
  ...
  - [배치를 7일치로 늘리자 화면이 6일 뒤 예측을 "지금"으로 보여줬다](#배치를-7일치로-늘리자-화면이-6일-뒤-예측을-지금으로-보여줬다)
...
**결론** — 코드는 규격대로였다. 남은 건 키뿐이었고, 그 키는 **이미 갖고 있었다.**

**data.go.kr 은 계정당 인증키가 하나다.** 서비스마다 키를 따로 받는 게 아니라,
**같은 인증키로 활용신청한 서비스 전부를 호출한다.** 그래서 KTO(관광공사) 호출에
쓰던 `KTO_API_KEY` 를 그대로 기상청 엔드포인트에 넣어보니 바로 통했다.

```
resultCode: 00  NORMAL_SERVICE
```

`KmaClient` 를 실제 데이터로 돌려 24시간 기온·강수확률 파싱까지 확인했다.
`KMA_API_KEY` 에 같은 값을 넣는 것으로 전환 완료 — 코드 변경 0줄.

**교훈** — data.go.kr 에서 403 을 만나면 **키를 새로 받을 일이 아니라
해당 서비스에 활용신청이 됐는지**를 본다. 이미 다른 공공 API 를 쓰고 있다면
키는 새로 필요하지 않다. 처음에 이걸 몰라서 "기상청 키를 따로 받아야 한다"고
생각했고, 그 오해가 OpenWeatherMap 우회로 이어졌다.
...
## 배치를 7일치로 늘리자 화면이 6일 뒤 예측을 "지금"으로 보여줬다

**증상** — 오늘(8/20) 경복궁 상세에 `16시 기준 · 혼잡 지수 44 여유`.
그런데 그 44 는 **8/26 의 16시 값**이었다. 실제 오늘 값은 `55 보통`이다.
등급이 한 칸 뒤집혀 "여유롭다"고 잘못 안내하고 있었다.

**원인** — 조회 쿼리가 `forecast_date` 내림차순 1건을 집는데 **상한이 없었다.**

```ts
.order('forecast_date', { ascending: false })   // 상한 없음
.limit(1)
```

원래 의도는 주석에 이렇게 적혀 있었다.

> 오늘치가 없으면 가장 최근 예측치라도 보여주기 위함 (PRD ⑦ 가용성)

즉 의도는 **과거로** 물러나기였다. 배치가 하루치만 쓸 때는 맞는 코드였다.
그런데 배치를 7일치 선계산으로 늘리자 **미래 날짜가 항상 최댓값**이 되어
내림차순 1건이 늘 다음 주를 집었다. 조회 코드는 한 줄도 안 바뀌었는데
**쓰기 쪽 변경이 읽기 쪽을 조용히 뒤집었다.**

**해결** — 4개 화면(상세·검색결과·대안비교·관심장소함)에 오늘 상한을 걸었다.

```ts
.lte('forecast_date', today)                    // 오늘 이하로 제한
.order('forecast_date', { ascending: false })
```

`utils/storedForecast.ts` 는 처음부터 `.lte()` 가 있어 멀쩡했다.
같은 저장소 안에 맞는 코드와 틀린 코드가 공존했던 셈이다.

**교훈** — **"최신"과 "미래"는 다르다.** `ORDER BY date DESC LIMIT 1` 은
"가장 최근"이 아니라 "가장 큰 날짜"다. 미래 행이 들어올 수 있는 테이블에서는
반드시 상한을 함께 건다.

그리고 **선계산 기간을 늘리는 변경은 조회 쿼리 전수 점검을 동반해야 한다.**
쓰기 범위가 넓어지면 읽기 쪽 가정이 깨진다. 이 버그는 에러도 안 나고
화면도 정상으로 보여서, 값을 하나씩 DB 와 맞춰보기 전까지 드러나지 않았다.

**발견 경로** — 배포 후 상세 화면 HTML 을 훑다가 `기준일 2026-08-26` 이
눈에 걸렸다. 그 문구는 예측이 낡았을 때만(`isStale`) 뜨게 해 둔 것인데,
오늘 데이터가 DB 에 있는데도 떠 있었다. **낡음 표시가 버그를 잡아 줬다.**
